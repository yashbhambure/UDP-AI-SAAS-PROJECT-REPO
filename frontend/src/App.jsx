...
function AppContent() {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden relative">
      ...
      <div className="absolute top-[-100px] left-[-100px] w-[600px] h-[600px] bg-purple-900/10 rounded-full blur-[160px] pointer-events-none z-0" />
      <div className="absolute bottom-[-100px] right-[-100px] w-[600px] h-[600px] bg-indigo-900/10 rounded-full blur-[160px] pointer-events-none z-0" />
              ...
              {filteredSearchItems.length === 0 ? (
                ...
                filteredSearchItems.map((item, idx) => (
                  <button
                    ...
                    className="w-full text-left flex items-center justify-between p-2.5 hover:bg-slate-800/60 rounded-xl transition-all cursor-pointer group"
                  >
                    <span className="text-xs font-semibold text-slate-200 group-hover:text-white truncate">{item.name}</span>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="sticky top-0 h-[72px] bg-slate-950/80 backdrop-blur-md border-b border-slate-900/60 px-8 flex items-center justify-between z-50 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center justify-between gap-12 bg-slate-900 hover:bg-slate-900/80 border border-slate-850 hover:border-slate-750 px-3.5 py-1.5 rounded-xl cursor-pointer text-slate-500 hover:text-slate-300 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
              <div className="flex items-center gap-2.5">
                <Search className="w-4 h-4" />
                <span className="text-[11px] font-semibold">Search everything...</span>
              </div>
              <div className="flex items-center gap-0.5 text-[9px] bg-slate-950 border border-slate-800 px-1.5 py-0.5 rounded font-bold">
            ...
            <div className="relative">
              <button
                ...
                className="w-8 h-8 rounded-xl bg-slate-900 border border-slate-850 text-slate-400 hover:text-white flex items-center justify-center cursor-pointer transition-colors relative focus:outline-none focus:ring-2 focus:ring-purple-500"
              >
              ...
              {showNotifications && (
                <div className="absolute right-0 top-10 w-64 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl py-2 z-30 animate-in fade-in zoom-in-95 duration-200">
                  <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-3.5 py-1.5 border-b border-slate-800/80 pb-2">Recent Notifications</h5>
                  <div className="p-2 space-y-1">
                    <div className="p-2 hover:bg-slate-800/40 rounded-lg text-[10px] text-slate-300">
                      <strong>AI Extraction</strong> parsed "Prompt Engineering.pdf" successfully.
                      <span className="block text-[8px] text-slate-500 mt-1">10 minutes ago</span>
                    </div>
                    <div className="p-2 hover:bg-slate-800/40 rounded-lg text-[10px] text-slate-300">
                      <strong>Reminder Alarm</strong>: Grant deadline is due in 3 days.
